...
//  c.moveTo(x_pos, y);
//  for (i = x_pos; i < 2000; i++) {
//    y = ((canvas.height / 2) +((-2*amp)/pi)*(Math.atan((1/(Math.tan(pi*(i - x_pos) *(time))) )))-(y_pos * 1));
//    c.lineTo(i, y);
//  }
//  c.stroke();
// }
    ...
    for (i = x_pos; i < 2000; i++) {
      if(signal_no == 1){
        ...
        y = ((canvas.height / 2 - 10) + ((Math.cos((i - x_pos) * 0.01 * frequency)) * (-amp)) - (y_pos * 1));
         ...
         y = ((canvas.height / 2) +((2*amp)/pi)*(Math.acos(((Math.cos(pi*(i - x_pos) *(2*frequency))) )))-(y_pos * 1));
      }
      else if(signal_no==5){
          y = ((canvas.height / 2) +Math.sign(((Math.sin(2*pi*(i - x_pos) * 0.001 * frequency)) * (amp)))*(amp * 1)-(y_pos*1));
      }
      c.lineTo(i, y);
    }
    c.stroke();
...
function remove_wire(){
  ...
  connect = 0;
  plot_condn();
  if(tour_on==13){tour_on=0;display.close()}
}
//FUNCTION GENERATOR----------------------------------------------------------------------\
var display = document.getElementById("display_canvas");
var disp = display.getContext("2d");
...
function signal_selection(){
  if(on_fg == 1 ){
      ...
      // else{func ="Cos";}
      if(signal_no==1){func="Sin";}
      else if(signal_no ==2){func="Cos";}
      else if(signal_no ==3){func ="Sawtooth";}
      else if(signal_no ==4){func ="Traiangle";}
      else if(signal_no ==5){func ="Square"}
      display_fg();
  }
  if(on==1){  window.requestAnimationFrame(plotSine_t);}
  if(tour_on==4){dialog.close();
